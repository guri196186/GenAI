...
#a = extract_video_id.run("https://www.youtube.com/watch?v=hfIUstzHs9A")
#print(a)
...
# a = fetch_transcript.run("hfIUstzHs9A")
# print(a)
...
#By appending this message to our conversation history, we're informing the LLM about 
# the results of the tool execution, which it can use in its next response.
...
""" Same workflow using LangChain's chain functionality"""
...
"""
The first step converts the user query into a properly formatted HumanMessage object
The second step sends this initial message to your tool-equipped LLM and adds the LLM's first response to the message history
The final step passes the conversation to your recursive chain, which will handle all subsequent tool calls until the LLM provides a final answer
"""
universal_chain = (
